...
const Activitepoppup: React.FC<ActivitepoppupProp> = ({ close, dh }) => {
  ...
  const addActivity = () => {
    if (newActivity && !activities.some(activity => activity._id === newActivity._id)) {
    ...
    <div className="z-50 fixed inset-0 flex items-center justify-center">
      ...
      <div className="flex items-center justify-center bg-white relative z-10 shadow-2xl rounded-3xl">
        <form
          ...
          className="w-[500px] p-6 rounded-lg"
        >
            ...
            {open && (
              <ul className="absolute top-32 mt-1 w-[80%] bg-white rounded-md shadow-md max-h-60 overflow-auto">
                {filteredActivities.map((activity) => (
                  ...
                  >
                    {activity.ref} {activity.activites.name} {activity.customer.firstname}{' '}
            ...
            <table className="w-full border-collapse">
              <tbody>
                {activities.map((activity, index) => (
                  <tr key={index} className="bg-purple-100 text-purple-900 text-sm border-b-2">
                    <td className="p-2 flex items-center gap-3 justify-between">
                      ...
                      <span>
                        A: {activity.activites.name} / C : {activity.customer.firstname}{' '}
                        {activity.customer.lastname} <br/>/ F : {activity.worker.name}
                      </span>
                      <button
                        type="button"
