...
const ActiviteTable: React.FC = () => {
  ...
  const updateStatus = async (event: React.ChangeEvent<HTMLSelectElement>, taskId:string) => {
    const status = event.target.value;

    try {
      const response = await fetch(`/api/accountant/task/updatepaystatus/${taskId}`, {
        method: 'PUT',
        headers: {
            ...
            tasks.map((task, index) => (
              <tr key={task._id} className="border-t hover:bg-gray-50">
                ...
                <td className="py-2 px-4">
                  <select
                    className={`mt-1 block w-[60%] px-2 py-2 text-white border  border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-500 focus: ${task.statuspay ? 'bg-green-500' : 'bg-red-500'}`}
                    value={
                      task.statuspay
                        ? "Paiement terminé"
                        : "En-coure de paiement"
                    }
                    onChange={(event)=>updateStatus(event,task._id)} 
                  >
                    <option value="En-coure de paiement">
                      En-coure de paiement
                    </option>
                    <option value="Paiement terminé">Paiement terminé</option>
                  </select>
                </td>
              </tr>
            ))
