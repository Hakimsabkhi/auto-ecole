...
const UpdateWorker=({ params }: { params: Promise<{ id: string }> }) =>{
   ...
   useEffect(() => {
      if (unwrappedParams?.id) {
        const fetchWorker = async () => {
          try {
            const response = await fetch(`/api/company/worker/getworkingbycompanywithid/${unwrappedParams.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        const response = await fetch(`/api/company/worker/updateworker/${unwrappedParams?.id}`, {  // Replace with your API endpoint
          method: 'PUT',
          headers: {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md w-full">
      <h2 className="text-2xl font-semibold text-center mb-4">Update Information Worker</h2>
