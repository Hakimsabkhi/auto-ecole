...
const CustomerForm: React.FC = () => {
 ...
 if (name === "cin" && value.length > 8) return;
  ...
  const handleWorkerSelect = (workerId: string) => {
    setFormData((prev) => {
      const isSelected = prev.worker.includes(workerId);
      const updatedWorkers = isSelected
        ? prev.worker.filter((id) => id !== workerId)
    ...
    <span className="text-gray-700">DT</span>
     ...
     <div className="grid grid-cols-8 gap-2">
          ...
          {workers.map((worker) => (
            <div
              key={worker._id}
              className={`cursor-pointer p-2 rounded-md border ${formData.worker.includes(worker._id) ? 'bg-blue-100 border-blue-500' : 'border-gray-300'} hover:bg-blue-50`}
              onClick={() => handleWorkerSelect(worker._id)}
            >
              <span className={`text-sm ${formData.worker.includes(worker._id) ? 'text-blue-600' : 'text-gray-700'}`}>
