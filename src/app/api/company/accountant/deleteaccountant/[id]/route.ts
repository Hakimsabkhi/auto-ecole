  ...
  try {
   ...
   const { id } = await context.params;
    ...
    if(!existingAccountant){
        return NextResponse.json({ message: '  existingAccountant not exist ' }, { status: 405 });
    }
    // Check if the Accountant exists
    const deletedAccountant= await Accountant.findByIdAndDelete(id);
    if (!deletedAccountant) {
      return NextResponse.json({ message: ' Accountant not found deleting' }, { status: 406 });
    }

    // Return success response
    return NextResponse.json({ message: 'Accountant deleted successfully' }, { status: 200 });
  } catch (error) {
